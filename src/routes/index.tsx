...
/** Compact one-tap upsell card reused by all three suggestion blocks. */
function UpsellCard({ item, onPick }: UpsellItem) {
  ...
  const from = item.variants.length > 0 ? Math.min(...item.variants.map((v) => v.price)) : item.price;
  return (
    <button
      type="button"
      onClick={onPick}
      disabled={!servable}
      aria-label={`${needsOptions ? "Choose options for" : "Add"} ${item.name}, ${formatPrice(from)}`}
      className="w-40 flex-shrink-0 text-left border border-brand-black/10 hover:border-brand-black transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
    >
      <div className="aspect-[4/3] overflow-hidden bg-stone-100">
        <img src={resolveImg(item.imageKey)} alt={item.name} loading="lazy" width={320} height={240} className="w-full h-full object-cover" />
      </div>
      <div className="p-2.5">
        <div className="font-bold text-xs uppercase tracking-tighter leading-tight line-clamp-2">{item.name}</div>
        <div className="font-mono text-[10px] mt-1 flex items-center justify-between">
          <span className="font-bold">{item.variants.length > 0 ? `From ${formatPrice(from)}` : formatPrice(from)}</span>
          <span className="text-brand-red font-bold uppercase">{!servable ? "N/A" : needsOptions ? "Pick" : "+ Add"}</span>
        </div>
      </div>
    </button>
  );
  ...
  return (
    <section className="mb-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
        {items.map((i) => (
          <UpsellCard key={i.id} item={i} onPick={() => onPick(i)} />
        ))}
      </div>
    </section>
  );
...
const QUICK_NOTES = ["Extra spicy", "No onion", "Less salt", "Extra sauce", "Cut into 8 pieces", "No ice"];
...
function OptionsModal({
  ...
  onClose: () => void;
  onSwitchItem: (i: PublicMenuItem) => void;
}) {
  const hasVariants = item.variants.length > 0;
  ...
  // Reset every control when the customer jumps to another product.
  useEffect(() => {
  ...
  const pickList = useCallback(
    (ids: string[]) => ids.map((id) => byId.get(id)).filter((i): i is PublicMenuItem => !!i && i.id !== item.id),
    [byId, item.id],
  );
  ...
  useEffect(() => setMealOn(false), [item.id]);
  ...
  const mealPrice = servableMeal.reduce((s, m) => s + (m.variants.length > 0 ? Math.min(...m.variants.map((v) => v.price)) : m.price), 0);
  ...
  /** Quick-add a suggestion, or open it when it needs options. */
  const pickSuggestion = (i: PublicMenuItem) => {
    if (i.variants.length > 0 || i.addons.length > 0) {
      onSwitchItem(i);
      return;
    }
    addEntry({
    ...
    if (mealOn) {
      for (const m of servableMeal) {
        addEntry({
          ...
          addonIds: [],
          addonNames: [],
          unitPrice: m.variants.length > 0 ? Math.min(...m.variants.map((v) => v.price)) : m.price,
          quantity: 1,
        });
      }
    }
  ...
  return (
    ...
    >
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`${item.name} — choose options`}
        tabIndex={-1}
        ...
        className="relative w-full sm:max-w-lg bg-white max-h-[92dvh] overflow-y-auto shadow-2xl outline-none pb-24"
      >
        <button
          onClick={onClose}
          className="absolute top-3 right-3 size-11 grid place-items-center bg-brand-black text-white hover:bg-brand-red transition-colors z-10"
        ...
        <div className="p-5 md:p-8">
          ...
          <h3 className="font-display text-3xl md:text-4xl uppercase tracking-tighter leading-none mb-3">{item.name}</h3>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[10px] uppercase tracking-wider text-brand-black/55 mb-5">
            {item.prepTimeMinutes !== null && (
              <span className="flex items-center gap-1"><Clock className="size-3" /> Ready in ~{item.prepTimeMinutes} min</span>
            )}
            <span className={`flex items-center gap-1 font-bold ${servable ? "text-emerald-700" : "text-brand-red"}`}>
              {servable ? <Check className="size-3" /> : <X className="size-3" />}
              {!item.inStock ? "Sold out" : onSchedule ? "Available now" : scheduleText ? `Serving ${scheduleText}` : "Not serving now"}
            </span>
            <span className="text-brand-black font-bold">{formatPrice(basePrice)}</span>
          </div>
          ...
          {hasVariants && (
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label={item.variantLabel}>
                {item.variants.map((v) => {
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-14 p-3 text-left border transition-colors ${
                        active ? "border-brand-red bg-brand-red text-white" : "border-brand-black/10 hover:border-brand-black"
                      }`}
                    >
                ...
                {item.addons.map((a) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={blocked}
                      className={`w-full min-h-12 flex items-center justify-between p-3 border transition-colors text-left ${
                        active
          ...
          {mealItems.length > 0 && (
            <div className="mb-6">
              <div className="font-mono text-[10px] font-bold uppercase tracking-[0.25em] mb-3">{item.mealUpgradeLabel}</div>
              <button
                type="button"
                role="checkbox"
                aria-checked={mealOn}
                onClick={() => setMealOn((v) => !v)}
                disabled={servableMeal.length === 0}
                className={`w-full min-h-14 flex items-center justify-between gap-3 p-4 border transition-colors text-left ${
                  mealOn ? "border-brand-gold bg-brand-gold text-brand-black" : "border-brand-black/10 hover:border-brand-black"
                } disabled:opacity-40 disabled:cursor-not-allowed`}
              >
                <span className="flex items-center gap-3 min-w-0">
                  <span className={`size-4 shrink-0 grid place-items-center border ${mealOn ? "border-brand-black bg-brand-black text-brand-gold" : "border-brand-black/30"}`}>
                    {mealOn && <Check className="size-3" />}
                  </span>
                  <span className="min-w-0">
                    <span className="block font-bold text-sm uppercase tracking-tighter truncate">
                      Add {servableMeal.map((m) => m.name).join(" + ") || "meal"}
                    </span>
                    <span className="block font-mono text-[10px] uppercase text-brand-black/60">One tap, everything added</span>
                  </span>
                </span>
                <span className="font-mono text-sm font-bold whitespace-nowrap">+{formatPrice(mealPrice)}</span>
              </button>
            </div>
          )}

          <div className="mb-6">
            <label htmlFor="pff-notes" className="block font-mono text-[10px] font-bold uppercase tracking-[0.25em] mb-3">
              Special instructions <span className="text-brand-black/40">(optional)</span>
            </label>
              ...
              {QUICK_NOTES.map((n) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 font-mono text-[10px] font-bold uppercase border transition-colors ${
                      on ? "bg-brand-black text-white border-brand-black" : "border-brand-black/15 hover:border-brand-black"
                    }`}
                  >
                    {n}
                  </button>
            ...
            <textarea
              ...
              className="w-full border border-brand-black/10 px-4 py-3 text-sm outline-none focus:border-brand-red transition-colors resize-none"
            />
          </div>
          ...
          <div className="flex items-center justify-between border-t border-brand-black/10 pt-6 mb-6">
            ...
            <div className="flex items-center border border-brand-black/10">
              ...
              <span className="w-12 text-center font-mono font-bold" aria-live="polite">{qty}</span>
              <button
          ...
          <div className="bg-stone-50 border border-brand-black/10 p-4 mb-6 font-mono text-[11px] uppercase tracking-wider space-y-1">
            <div className="flex justify-between"><span className="text-brand-black/50">Item</span><span className="font-bold">{item.name}</span></div>
            {variant && <div className="flex justify-between"><span className="text-brand-black/50">{item.variantLabel}</span><span className="font-bold">{variant.name}</span></div>}
            {selectedAddons.length > 0 && (
              <div className="flex justify-between gap-4"><span className="text-brand-black/50">{item.addonLabel}</span><span className="font-bold text-right">{selectedAddons.map((a) => a.name).join(", ")}</span></div>
            )}
            {mealOn && servableMeal.length > 0 && (
              <div className="flex justify-between gap-4"><span className="text-brand-black/50">Meal</span><span className="font-bold text-right">{servableMeal.map((m) => m.name).join(", ")}</span></div>
            )}
            {notes.trim() && <div className="flex justify-between gap-4"><span className="text-brand-black/50">Notes</span><span className="font-bold text-right normal-case">{notes.trim()}</span></div>}
            <div className="flex justify-between"><span className="text-brand-black/50">Quantity</span><span className="font-bold">{qty}</span></div>
            <div className="flex justify-between border-t border-brand-black/10 pt-1 mt-1"><span className="text-brand-black/50">Total</span><span className="font-bold text-brand-red">{formatPrice(total)}</span></div>
          </div>

          {added && (
            <p role="status" className="font-mono text-[10px] uppercase tracking-widest text-emerald-700 mb-4">{added} added to cart</p>
          )}

          <UpsellRow title="Frequently bought together" subtitle="Customers usually order this with:" items={frequently} onPick={pickSuggestion} />
          <UpsellRow title="You may also like" items={recommended} onPick={pickSuggestion} />
        </div>

        <div className="sticky bottom-0 left-0 right-0 bg-white border-t border-brand-black/10 p-4">
          <button
            onClick={handleAdd}
            disabled={missingVariant || !servable}
            className="w-full min-h-14 py-4 bg-brand-red text-white font-bold uppercase tracking-tighter text-sm hover:bg-brand-black transition-colors flex items-center justify-between px-6 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <span className="flex items-center gap-2">
              <Plus className="size-4" />
              {!servable ? (item.inStock ? "Not serving now" : "Sold out") : missingVariant ? `Select ${item.variantLabel}` : "Add to Cart"}
            </span>
