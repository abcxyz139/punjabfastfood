...
export const createMenuItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("menu_items").insert({
      ...
      slug: menuSlug(data.slug, data.name),
      gallery_keys: data.galleryKeys,
      ...
      recommended_ids: data.recommendedIds,
      frequently_bought_ids: data.frequentlyBoughtIds,
      meal_upgrade_ids: data.mealUpgradeIds,
      meal_upgrade_label: data.mealUpgradeLabel,
      badges: data.badges,
